...
span.songTop.pp, span.scoreTop.ppValue, span.scoreTop.ppLabel, span.songTop.mapper {
	...
	function get_document_user(doc) {
	    let username_elem = check(doc.querySelector(".content .title a"));
	    let user_name = username_elem.innerText.trim();
	    let user_id = Global.user_reg.exec(window.location.href)[1];
	...
	function check_for_updates(edit_elem) {
	    ...
	    GM_xmlhttpRequest({
	        ...
	        onload: function (response) {
	            ...
	            let latest_version = Global.script_version_reg.exec(latest_script)[1];
	...
	***************************************************************************** */
	...
	function get_song_hash_from_text(text) {
	    return check(Global.song_hash_reg.exec(text))[1];
	}
	function fetch_song_info_by_hash(hash) {
	    return __awaiter(this, void 0, void 0, function* () {
	        const data = JSON.parse(yield fetch2(Global.beatsaver_hash_api + hash));
	        return data;
	    });
	}
	function oneclick_install_byhash(song_hash) {
	    return __awaiter(this, void 0, void 0, function* () {
	...
	function generate_beatsaver_button(song_hash, size) {
	    let base_elem = create("div", {
	        ...
	        onclick: () => __awaiter(this, void 0, void 0, function* () {
	            const song_data = yield fetch_song_info_by_hash(song_hash);
	            new_page(Global.beatsaver_link + song_data.key);
	        }),
	    });
	...
	function generate_oneclick_button(song_hash, size) {
	    return create("div", {
	        ...
	        onclick: () => __awaiter(this, void 0, void 0, function* () {
	            yield oneclick_install_byhash(song_hash);
	        }),
	...
	function generate_bsaber_button(song_hash) {
	    return create("a", {
	        ...
	        onclick: () => __awaiter(this, void 0, void 0, function* () {
	            const song_data = yield fetch_song_info_by_hash(song_hash);
	            new_page(Global.bsaber_link + song_data.key);
	        }),
	    }, create("div", {
	        style: {
	...
	function setup_dl_link_user_site() {
	    ...
	    for (let row of table_row) {
	        let image_link = check(row.querySelector("th.song img")).src;
	...
	function setup_user_rank_link_swap() {
	    ...
	    let elem_global = check(document.querySelector(".content div.columns ul li a"));
	    let res_global = check(Global.leaderboard_rank_reg.exec(elem_global.innerText));
	...
	function setup_song_rank_link_swap() {
	    ...
	    for (let row of song_elems) {
	        ...
	        let leaderboard_link = check(row.querySelector("th.song a")).href;
	...
	function setup_song_filter_tabs() {
	    ...
	    function load_friends() {
	        ...
	        let song_id = Global.leaderboard_reg.exec(window.location.pathname)[1];
	        let elements = [];
	...
	function update_user_compare_songtable(other_user) {
	    ...
	    let other_data = Global.user_list[other_user];
	    if (!other_data) {
	    ...
	    for (let row of table_row) {
	        ...
	        if (other_song) {
	            other_score_content = create("div", {}, create("span", { class: "scoreTop ppValue" }, format_en(other_song.pp)), create("span", { class: "scoreTop ppLabel" }, "pp"), create("br"), (function () {
	                let str;
	...
	function get_row_data(row) {
	    ...
	    let leaderboard_elem = check(row.querySelector("th.song a"));
	    ...
	    let song_id = Global.leaderboard_reg.exec(leaderboard_elem.href)[1];
	    let pp = Number(pp_elem.innerText);
	    let time = time_elem.title;
	    let score = undefined;
	    let accuracy = undefined;
	    let mods = undefined;
	    let score_res = check(Global.score_reg.exec(score_elem.innerText));
	    logc(score_res);
	...
	function fetch_user_page(id, page) {
	    return __awaiter(this, void 0, void 0, function* () {
	        let link = Global.scoresaber_link + `/u/${id}&page=${page}&sort=2`;
	        if (window.location.href.toLowerCase() === link) {
	            logc("Efficient get :P");
	            return document;
	        }
	        logc(`Fetching user ${id} page ${page}`);
	        let init_fetch = yield (yield fetch(link)).text();
