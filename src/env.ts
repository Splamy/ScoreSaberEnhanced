...
export function get_document_user(doc: Document) {
    ...
    let user_id = g.user_reg.exec(window.location.href)![1];
...
export function get_compare_user(): string | undefined {
    ...
    let compare = document.getElementById("user_compare") as HTMLSelectElement | null;
...
export function set_compare_user(user: string) {
    g.last_selected = user;
    localStorage.setItem("last_selected", user);
}

export function set_home_user(user: IUser) {
    g._home_user = user;
    localStorage.setItem("home_user", JSON.stringify(user));
}

export function set_wide_table(value: boolean) {
...
export function set_show_bs_link(value: boolean) {
...
export function set_show_oc_link(value: boolean) {
